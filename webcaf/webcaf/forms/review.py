...
class CompanyDetailsForm(ModelForm):
    """
    Handles the company details form with fields for name, email, address, and phone number.

    This class is used to define a form for capturing details of a company including its name,
    email, address, and phone number. The class allows for form initialization with custom
    initial data and implements cleaning logic to structure the cleaned data with specific keys.

    :ivar company_name: The name of the company.
    :type company_name: CharField
    :ivar company_email: The email address of the company.
    :type company_email: EmailField
    :ivar company_address: The physical address of the company, optional.
    :type company_address: CharField
    :ivar company_phone: The contact phone number of the company, optional.
    :type company_phone: CharField
    """
    ...
    def clean(self):
        """
        Cleans and formats the `text` field with relevant fields from cleaned data.

        This method processes the `cleaned_data` dictionary to create a new value
        for the `text` field. The `text` field is constructed as a dictionary
        containing the `company_name` and `company_email` keys with their
        corresponding values.

        :raises KeyError: If required keys are not present in `cleaned_data`.
        :return: None
        """
        self.cleaned_data["text"] = {
