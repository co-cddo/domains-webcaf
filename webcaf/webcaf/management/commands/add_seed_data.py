    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username=SUPERUSER_NAME).exists():
            superuser = User.objects.create_superuser(
                ...
                email=f"{SUPERUSER_NAME}@example.com",
                password=SUPERUSER_PASSWORD
            )

            UserProfile.objects.create(user=superuser)
            self.stdout.write(self.style.SUCCESS(
                f"Superuser '{SUPERUSER_NAME}' created"
            ))
        else:
        ...
        if not User.objects.filter(username=USER_NAME).exists():
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Standard user '{USER_NAME}' created"
            ))
        else:
        ...
        for system_name in SYSTEM_NAMES:
            _, sys_created = System.objects.get_or_create(
                name=system_name,
                organisation=organisation
            )
            if not sys_created:
                self.stdout.write(self.style.WARNING(
                    f"System '{system_name}' already exists for organisation '{ORG_NAME}'"
                ))
